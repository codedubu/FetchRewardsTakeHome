...
class NetworkManager {
    ...
    func getAllMealCategories(completion: @escaping (Result<[Category], RBError>) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: finalURL) { data, response, error in
            ...
            do {
                ...
                let categorySearchResults = try decoder.decode(CategorySearchResults.self, from: data)
                
                completion(.success(categorySearchResults.categories))
            } catch {
    ...
    func getAllMeals(meal: String, completion: @escaping (Result<[Meal], RBError>) -> Void) {
        guard let baseURL = URL(string: baseURL) else { return completion(.failure(.invalidURL)) }
        ...
        let task = URLSession.shared.dataTask(with: finalURL) { data, response, error in
            ...
            do {
                let decoder = JSONDecoder()
                let mealSearchResults = try decoder.decode(MealSearchResults.self, from: data)
                
                
                completion(.success(mealSearchResults.meals))
            } catch {
                print("Error in \(#function) : \(error.localizedDescription) \n---\n \(error)")
                return completion(.failure(.thrownError(error)))
            }
    ...
    func getAllMealDetails(for id: String, completion: @escaping (Result<MealDetail, RBError>) -> Void) {
        guard let baseURL = URL(string: baseURL) else { return completion(.failure(.invalidURL)) }
